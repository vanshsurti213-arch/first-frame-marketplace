...
export default function BrandCampaignPage() {
        ...
        <div className="max-w-md w-full text-center animate-fade-in">
          <div className="glass-card p-8">
            ...
            <p className="text-sm text-white/40 mb-6 leading-relaxed">
              Hi <strong className="text-white">{brand?.companyName || "there"}</strong>! We are currently configuring your brand campaign dashboard. 
            ...
            <div className="flex flex-col gap-2">
              ...
              <button 
                ...
                className="w-full py-3 rounded-xl bg-transparent border border-white/[0.08] text-white/40 text-sm font-semibold hover:bg-white/[0.02] transition-colors"
              >
                Log Out
              </button>
            </div>
  ...
  const METRIC_CARDS = [
    { label: "Invited", value: metrics?.invitedCreators ?? 0, icon: <UserPlus className="w-5 h-5 text-[#3B82F6]" /> },
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3 mt-2">
          {campaign?.collab_type && (
            <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-white/[0.08] text-white">
              {campaign.collab_type}
            </span>
          )}
        ...
        {METRIC_CARDS.map((card) => (
          <div key={card.label} className="glass-card p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-semibold uppercase tracking-wider text-white/40">{card.label}</span>
              {card.icon}
            </div>
            <div className="font-display font-bold text-3xl text-white">{card.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-3 mb-8">
        <button onClick={() => router.push("/brand/creators")} className="btn-primary text-sm">
          <Users className="w-4 h-4" /> Browse Creators
        </button>
        <button onClick={() => router.push("/brand/campaign/products/new")} className="btn-ghost text-sm">
      ...
      {pendingReviewCount > 0 && (
        <div
          ...
          className="glass-card p-5 mb-8 cursor-pointer border-l-4 border-l-[#FFB547] hover:shadow-lg transition-shadow"
        >
          <div className="flex items-center gap-3">
            <FileVideo className="w-5 h-5 text-[#FFB547]" />
            <div>
              <p className="text-sm font-semibold text-white">{pendingReviewCount} video{pendingReviewCount !== 1 ? "s" : ""} awaiting your review</p>
              <p className="text-xs text-white/40">Click to review submissions</p>
            </div>
      ...
      <div className="mb-4">
        <h2 className="font-display font-bold text-lg text-white mb-4">Recent Creators</h2>
      </div>
      {creators.length === 0 ? (
        <EmptyState icon={<Users className="w-7 h-7 text-white/25" />} title="No creators yet" description="Start by browsing and inviting creators to your campaign." />
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.08]">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider text-white/40">Since</th>
              </tr>
            </thead>
            <tbody>
              {creators.slice(0, 5).map((cc) => (
                <tr key={cc.id} className="table-row-hover border-b border-white/[0.04]">
                  <td className="px-5 py-4 text-sm font-medium text-white">{cc.creator_name.split(" ")[0]}</td>
