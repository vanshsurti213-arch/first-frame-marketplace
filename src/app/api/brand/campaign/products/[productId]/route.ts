...
export async function GET(
  ...
  try {
    ...
    console.error('[BRAND PRODUCT GET ERROR]', err);
...
export async function PATCH(
  ...
  try {
    ...
    // Check if script_content is being updated
    ...
    if (isScriptUpdate && updatedProduct.assigned_creator_ids?.length > 0) {
      ...
      if (creatorsToNotify) {
        for (const cc of creatorsToNotify) {
           ...
           // Automatically update status from product_dispatched to brief_received if needed
    ...
    console.error('[BRAND PRODUCT PATCH ERROR]', err);
    return NextResponse.json({ success: false, error: 'An unexpected error occurred' }, { status: 500 });
  }
}
